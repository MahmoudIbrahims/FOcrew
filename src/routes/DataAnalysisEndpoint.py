from Agents.Prompts import (data_reader_prompt,cleaning_prompt,
                                analysis_prompt,visualizations_prompt,report_writer_prompt,
                                            Convert_md_to_pdf_prompt,SendEmail_prompt)

from Agents import (DataReaderAgent,DataCleanerAgent,DataAnalyzerAgent,
                            DataVisualizerAgent,ReportWriterAgent,ReportGeneratorAgent,ReportSenderAgent)
...
async def inventory_agent(
    ...
    Report_Writer_task.description =report_writer_prompt.safe_substitute(working_dir=job_dir_path.as_posix(),report_md_path =final_md_path.as_posix(),target_language=DataAnalysis_Request.Language)
