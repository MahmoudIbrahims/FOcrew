...
export const AnalysisSection: React.FC<AnalysisSectionProps> = ({ projectId, isUploadComplete, onAnalysisSuccess }) => {
  ...
  const handleAnalyze = async () => {
    if (!projectId) {
      setErrorMsg('Project ID is missing. Please select a project.');
    ...
    try {
      ...
      const pdfUrl = await runAnalysis(projectId, payload);
      ...
      setErrorMsg(err.message || 'Analysis failed. Make sure the backend is running.');
    }
  };
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden h-full flex flex-col">
      ...
      <div className="p-6 flex-1 flex flex-col space-y-5">
        ...
        {/* Industry Name */}
        <div>
          <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1.5">Industry Name</label>
          <input
            type="text"
            name="INDUSTRY_NAME"
        ...
        {/* Language Selection */}
        <div>
          <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1.5">Report Language</label>
          <select
            name="Language"
        ...
        {status === AnalysisStatus.ERROR && (
          <div className="p-3 bg-red-50 text-red-700 rounded-md text-sm flex items-start border border-red-100">
        ...
        {status === AnalysisStatus.SUCCESS && (
           <div className="p-3 bg-green-50 text-green-700 rounded-md text-sm flex items-center border border-green-100 animate-pulse">
             <Sparkles className="w-4 h-4 mr-2" />
             <span>Analysis complete! Preparing report...</span>
           </div>
        )}
        ...
        <div className="mt-auto pt-4">
          ...
          <button
            ...
            className={`w-full py-3 px-4 rounded-lg font-bold text-sm flex items-center justify-center transition-all shadow-md ${
